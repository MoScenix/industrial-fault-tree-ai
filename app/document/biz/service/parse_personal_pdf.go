...
type ParsePersonalPDFService struct {
	ctx context.Context
} // NewParsePersonalPDFService new ParsePersonalPDFService
func NewParsePersonalPDFService(ctx context.Context) *ParsePersonalPDFService {
	return &ParsePersonalPDFService{ctx: ctx}
}

// Run create note info
func (s *ParsePersonalPDFService) Run(req *document.ParsePersonalPDFReq) (resp *document.ParsePDFResp, err error) {
	if req.GetUserId() == "" || req.GetPdfId() == "" {
		return &document.ParsePDFResp{
			Success:      false,
			ErrorMessage: "user_id and pdf_id are required",
		}, nil
	}
